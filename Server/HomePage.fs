...
module Home =
                  ...
                  [ Attr.classes [ "dropdown-trigger" ]
                    Html.button
                        [ Attr.id "dropdown_button"
                          ...
                          Html.span [ Attr.id "dropdown_button_text"; Html.text "All Types" ]
                          Html.span
                    ...
                    Html.div
                        [ Attr.classes [ "dropdown-menu" ]
                          ...
                          Attr.role "menu"
                          Html.div [ Attr.classes [ "dropdown-content"; "has-text-left" ]; Fragment dropdownItems ] ] ] ]
              ...
              | Some i ->
                  Html.div
                      [ Attr.classes [ "card-image" ]
                        Html.figure [ Attr.classes [ "image"; "is4by3" ]; Html.img [ Attr.src i ] ] ]
              | _ -> ()
    ...
    let getPostSummaries posts =
        posts |> Seq.sortBy (fun p -> p.Title) |> Seq.map (fun p -> buildPostCard p)

    let getColumnPosts posts colNo totCols =
        posts
        |> Seq.mapi (fun i p -> i, p)
        |> Seq.filter (fun (i, _) -> [ colNo..totCols..1000 ] |> Seq.contains (i + 1))
        |> Seq.map snd
            ...
            [ Attr.classes [ "container" ]
              Html.div
                  [ Attr.classes [ "columns" ]
                    Html.div [ Attr.classes [ "column"; "is-four-fifths" ]; (buildTagList posts) ]
                    Html.div
