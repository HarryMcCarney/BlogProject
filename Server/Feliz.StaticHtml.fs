...
let Html =
    HtmlEngine((fun tag nodes -> El(tag, List.ofSeq nodes)), Text, (fun () -> Fragment []))

let Svg =
    SvgEngine((fun tag nodes -> El(tag, List.ofSeq nodes)), Text, (fun () -> Fragment []))

let Attr =
    AttrEngine((fun k v -> Attr(k, Choice1Of2 v)), (fun k v -> Attr(k, Choice2Of2 v)))
