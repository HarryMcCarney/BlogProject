...
module Util =
    ...
    let rec combinations acc size set =
        seq {
            ...
            | n, x :: xs ->
                if n > 0 then
                    yield! combinations (x :: acc) (n - 1) xs

                if n >= 0 then
                    yield! combinations acc n xs
            | 0, [] -> yield acc
            | _, [] -> ()
        }
...
module GameLogic =
        ...
        let rec playRound (game: Game) (rounds: int) (counter: int) : Game =
            if counter > rounds then
                ...
                let updatedGame: Game =
                    let p1Moves =
                        [| (game.Player1.Strategy game.Player1Moves game.Player2Moves) |]
                        |> Array.append game.Player1Moves

                    let p2Moves =
                        [| (game.Player2.Strategy game.Player2Moves game.Player1Moves) |]
                        |> Array.append game.Player2Moves
    ...
    let evaluateGame (game: Game) =
        ...
        |> fun (p1, p2) ->
            let evaluatedGame =
                { game with
                    Player1Score = p1 |> Array.sum |> Some
                    Player2Score = p2 |> Array.sum |> Some }
    ...
    let runTournament players =
        ...
        |> Seq.map (fun c -> play c[0] c[1] 200 |> evaluateGame)
        |> Seq.toArray
        |> fun games ->
    ...
    let getLine (results: GenerationResult array) strategyName =
        let x, y =
            results
            |> Array.map (fun gr -> gr.Generation, (gr.StrategyPopulation |> Map.find strategyName))
            |> Array.unzip
            ...
            if counter >= generations then
                ...
                let nextGen =
                    ...
                    |> fun scores ->
                        ...
                        |> Array.map (fun (p, s) -> [| 0..s |] |> Array.map (fun _ -> p.Recreate()))
                    |> Array.concat
...
module Strategies =
    ...
    let rec tester () : Player =
        ...
        let strategy =
            fun (playerMoves: Move array) (opponentMoves: Move array) ->
                if playerMoves.Length = 0 then
                    Defect
                elif playerMoves.Length <= 3 then
                    Cooperate
                elif not (opponentMoves |> Array.exists (fun m -> m = Defect)) then
                    if playerMoves.Length % 2 = 0 then Defect else Cooperate
                elif
                    opponentMoves |> Array.last = Defect
                    && opponentMoves |> Array.filter (fun m -> m = Defect) |> Array.length = 1
                then
                    Cooperate
                else
                    opponentMoves |> Array.last
    ...
    let rec grudger () : Player =
        ...
        let strategy =
            fun (playerMoves: Move array) (opponentMoves: Move array) ->
                if playerMoves.Length = 0 then
                    Cooperate
                else if opponentMoves |> Array.exists (fun m -> m = Defect) then
                    Defect
                else
                    Cooperate
    ...
    let rec alwaaysDefect () : Player =
        ...
        let strategy = fun _ _ -> Defect

        { Id = Guid.NewGuid()
          Name = name
          Strategy = strategy
          Recreate = alwaaysDefect }


    let rec alwaaysCooperate () : Player =
        let name = "AlwaaysCooperate"
        let strategy = fun _ _ -> Cooperate

        { Id = Guid.NewGuid()
          Name = name
          Strategy = strategy
          Recreate = alwaaysCooperate }
...
|> fun results -> players |> Array.map (fun p -> getLine results p.Name)
|> Chart.combine
